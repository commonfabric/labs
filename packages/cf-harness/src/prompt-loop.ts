  ...
  [
    ...
    ...(profileConfig.profile === PATTERN_AUTHOR_SUBAGENT_PROFILE
      ? [
        ...
        "A pending read is not data: its zeros and empty lists are placeholders. Check pending, error, and outputConcerns before returning a working page. Re-read the SAME result reference through run_pattern under the read's schema; do not create replacement pages to wait for data. For a settled empty filtered result, count the same source without the uncertain predicate and present both counts and the filter. Render loading, errors, and settled empty states distinctly. A verification probe has no user-facing UI and must stay unnamed.",
