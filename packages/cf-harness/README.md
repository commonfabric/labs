...
pattern id (`cf:pattern:` identifier, an id labeled `pattern`/`pattern id`, or a
bare content hash following an execution verb), or a named skill. Skill
selection recognizes instructions to use registered skills, addresses labeled
`skill`, and `https://skills.sh/` addresses. A bare three-part path is not a
skill selection. It does not acquire or validate the selection; the ordinary
tools do that. The parent can still call `research` for an unresolved question.
