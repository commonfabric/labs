// EXPERIMENT (own-entry, arm `r1-lazy`): a copy of
...
/** EXPERIMENT (r1-lazy): where the board keeps a topic's entry document
...
export interface TopicsInput {
  /** EXPERIMENT (r1-lazy): one slot per topic the board has filed. */
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (r1-lazy): one slot per topic, each holding that topic's own
   * entry document. */
  entrySlots: EntrySlot[] | Default<[]>;

  /** EXPERIMENT (r1-lazy): how many entries the last fill wrote. Demanding it
...
/** EXPERIMENT (r1-lazy): what the board holds about each topic, written
...
export default pattern<TopicsInput, TopicsOutput>((
  ...
  // EXPERIMENT (r1-lazy): what the board holds about each topic, written into
  ...
  const addTopic = action<AddTopicEvent, AddTopicResult>((
    ...
    // EXPERIMENT (r1-lazy): the name is allocated before the topic is
    // created, so the create can store it in the topic.
    const { name, member: piece } = createNamed(names, (allocated) => {
      // EXPERIMENT (r1-lazy): the entry document, minted by the create so the
      ...
      return Topic({
        ...
        mentionable,
        // EXPERIMENT (r1-lazy): this topic's own entry, and no table.
        ownEntry: entry,
      });
    });
