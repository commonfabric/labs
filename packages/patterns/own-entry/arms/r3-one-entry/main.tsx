// EXPERIMENT (own-entry, arm `r3-one-entry`): a copy of
// packages/patterns/topics/main.tsx with the edits
...
/** EXPERIMENT (r3-one-entry): where the board keeps a topic's entry document,
...
export interface TopicsInput {
  /** EXPERIMENT (r3-one-entry): one slot per topic the board has filed. */
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (r3-one-entry): one slot per topic, each holding that topic's own
   * entry document. */
  entrySlots: EntrySlot[] | Default<[]>;

  /** EXPERIMENT (r3-one-entry): the named pivot the fills read their rows from. */
  pivot: PivotRow[] | Default<[]>;

  /** EXPERIMENT (r3-one-entry): how many references each per-topic fill wrote.
...
/** EXPERIMENT (r3-one-entry): what the pivot reads of one topic — what it points
...
/** EXPERIMENT (r3-one-entry): one row of the pivot, addressed by the topic it is
...
/** EXPERIMENT (r3-one-entry): the board's mention pivot, named. One row per topic,
...
/** EXPERIMENT (r3-one-entry): the pivot indexed by the name each row carries — a
...
/** EXPERIMENT (r3-one-entry): ONE topic's entry, written from that topic's own row.
...
const fillOneEntry = lift((
  { entry, name, row, collectionName, universe }: {
    entry: Writable<OwnEntry>;
    name: string;
...
/** EXPERIMENT (r3-one-entry): the universe, bounded. A member's editor completes
...
export default pattern<TopicsInput, TopicsOutput>((
  ...
  // EXPERIMENT (r3-one-entry): the universe, bounded, and derived once.
  const universe = boundedUniverse({ rows: mentionable });
  // EXPERIMENT (r3-one-entry): the named pivot, indexed by name, and one entry per
  ...
  const addTopic = action<AddTopicEvent, AddTopicResult>((
    ...
    // EXPERIMENT (r3-one-entry): the entry document is minted before the topic,
    ...
    const { name, member: piece } = createNamed(names, (allocated) =>
      Topic({
        ...
        createdAt: Date.now(),
        createdBy: author,
        // EXPERIMENT (r3-one-entry): this topic's own entry, and nothing else of
        // the board.
        ownEntry: entry,
      }));
  ...
  return {
    ...
    // EXPERIMENT (r3-one-entry): the board declares the name its entries carry.
