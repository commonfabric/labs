...
export interface TopicInput {
  ...
  /** EXPERIMENT (q6-copies): the name the board allocated for this topic, stored
   * here by the create that filed it. */
  shortName?: string;

  /** EXPERIMENT (q6-copies): this topic's own entry, the reference the board
   * handed over when it filed the topic. */
  ownEntry?: ReadonlyCell<OwnEntry | undefined>;
}

/** EXPERIMENT (q6-copies): what a topic shows of a topic that mentions it. */
export interface TopicBacklinkCopy {
  title: string;
  shortName: string;
}

/** EXPERIMENT (q6-copies): what the board holds about one topic. */
 ...
 */
/** EXPERIMENT (q6-copies): the inbound references this topic's entry carries. */
...
export default pattern<TopicInput, TopicOutput>(
  (
    ...
    // EXPERIMENT (q6-copies): the name is this topic's own stored input.
