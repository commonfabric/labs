// EXPERIMENT (own-entry, arm `q6-copies`): a copy of
...
/** EXPERIMENT (q6-copies): where the board keeps a topic's entry document
...
export interface TopicsInput {
  /** EXPERIMENT (q6-copies): one slot per topic the board has filed. */
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (q6-copies): one slot per topic, each holding that topic's own
   * entry document. */
  entrySlots: EntrySlot[] | Default<[]>;

  /** EXPERIMENT (q6-copies): how many entries the last fill wrote. Demanding it
...
/** EXPERIMENT (q6-copies): what the board holds about each topic, written
...
export default pattern<TopicsInput, TopicsOutput>((
  ...
  // EXPERIMENT (q6-copies): what the board holds about each topic, written into
  ...
  const addTopic = action<AddTopicEvent, AddTopicResult>((
    ...
    // EXPERIMENT (q6-copies): the name is allocated before the topic is
    // created, so the create can store it in the topic.
    const { name, member: piece } = createNamed(names, (allocated) => {
      // EXPERIMENT (q6-copies): the entry document, minted by the create so the
      ...
      return Topic({
        ...
        mentionable,
        // EXPERIMENT (q6-copies): this topic's own entry, and no table.
        ownEntry: entry,
      });
    });
