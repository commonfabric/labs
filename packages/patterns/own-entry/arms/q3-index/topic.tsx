...
export interface TopicInput {
  ...
  /** EXPERIMENT (q3-index): the name the board allocated for this topic, stored
   * here by the create that filed it. */
  shortName?: string;

  /** EXPERIMENT (q3-index): the board's entries, indexed by the name each
   * carries. The topic looks up its own. */
  boardEntryIndex: KeyIndex<string, OwnEntry>;
}

/** EXPERIMENT (q3-index): what the board holds about one topic. */
 ...
 */
/** EXPERIMENT (q3-index): the inbound references this topic's entry carries. */
...
export default pattern<TopicInput, TopicOutput>(
  (
    ...
    // EXPERIMENT (q3-index): the name is this topic's own stored input.
