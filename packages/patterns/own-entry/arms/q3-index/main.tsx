// EXPERIMENT (own-entry, arm `q3-index`): a copy of
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (q3-index): one entry per named topic. */
  ownEntries: OwnEntry[];

  /** EXPERIMENT (q3-index): the entries indexed by name. */
...
/** EXPERIMENT (q3-index): one entry per named topic — what the board holds
...
/** EXPERIMENT (q3-index): the entries, indexed by the name each carries. */
...
export default pattern<TopicsInput, TopicsOutput>(({ topics, names }) => {
  ...
  // EXPERIMENT (q3-index): each topic's entry, derived once for the board, and
  ...
  const addTopic = action<AddTopicEvent, AddTopicResult>((
    ...
    // EXPERIMENT (q3-index): the name is allocated before the topic is
    ...
    const { name, member: piece } = createNamed(names, (allocated) =>
      Topic({
        ...
        mentionable,
        // EXPERIMENT (q3-index): the board's entry index, and no table.
