// EXPERIMENT (own-entry, arm `r2-per-entry`): a copy of
...
/** EXPERIMENT (r2-per-entry): where the board keeps a topic's entry document,
...
export interface TopicsInput {
  /** EXPERIMENT (r2-per-entry): one slot per topic the board has filed. */
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (r2-per-entry): one slot per topic, each holding that topic's own
   * entry document. */
  entrySlots: EntrySlot[] | Default<[]>;

  /** EXPERIMENT (r2-per-entry): the named pivot the fills read their rows from. */
  pivot: PivotRow[] | Default<[]>;

  /** EXPERIMENT (r2-per-entry): how many references each per-topic fill wrote.
...
/** EXPERIMENT (r2-per-entry): what the pivot reads of one topic — what it points
...
/** EXPERIMENT (r2-per-entry): one row of the pivot, addressed by the topic it is
...
/** EXPERIMENT (r2-per-entry): the board's mention pivot, named. One row per topic,
...
/** EXPERIMENT (r2-per-entry): the pivot indexed by the name each row carries — a
...
/** EXPERIMENT (r2-per-entry): ONE topic's entry, written from that topic's own row.
...
export default pattern<TopicsInput, TopicsOutput>((
  ...
  // EXPERIMENT (r2-per-entry): the named pivot, indexed by name, and one entry per
  ...
  const addTopic = action<AddTopicEvent, AddTopicResult>((
    ...
    // EXPERIMENT (r2-per-entry): the entry document is minted before the topic,
    ...
    const { name, member: piece } = createNamed(names, (allocated) =>
      Topic({
        ...
        mentionable,
        // EXPERIMENT (r2-per-entry): this topic's own entry, and nothing else of
        // the board.
        ownEntry: entry,
      }));
