...
export interface TopicInput {
  ...
  /** EXPERIMENT (r2-per-entry): the name the board allocated for this topic, stored
   * here by the create that filed it. */
  shortName?: string;

  /** EXPERIMENT (r2-per-entry): this topic's own entry, the reference the board
   * handed over when it filed the topic. */
  ownEntry?: ReadonlyCell<OwnEntry | undefined>;
}

/** EXPERIMENT (r2-per-entry): what a topic shows of a topic that mentions it. */
export interface TopicBacklinkCopy {
  title: string;
  shortName: string;
}

/** EXPERIMENT (r2-per-entry): what the board holds about one topic. */
 ...
 */
/** EXPERIMENT (r2-per-entry): the inbound references this topic's entry carries. */
...
export default pattern<TopicInput, TopicOutput>(
  (
    ...
    // EXPERIMENT (r2-per-entry): the name is this topic's own stored input.
