// EXPERIMENT (own-entry, arm `q7-board-name`): a copy of
...
/** EXPERIMENT (q7-board-name): where the board keeps a topic's entry document
...
export interface TopicsInput {
  /** EXPERIMENT (q7-board-name): one slot per topic the board has filed. */
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (q7-board-name): one slot per topic, each holding that topic's own
   * entry document. */
  entrySlots: EntrySlot[] | Default<[]>;

  /** EXPERIMENT (q7-board-name): how many entries the last fill wrote. Demanding it
...
/** EXPERIMENT (q7-board-name): what the board holds about each topic, written
  ...
  { slots, rows, names }: {
    ...
    rows:
      | {
        ...
        mentionedBy: ReadonlyCell<{ title: string | Default<""> }>[];
      }[]
      | Default<[]>;
    // deno-lint-ignore ban-types
...
export default pattern<TopicsInput, TopicsOutput>((
  ...
  // EXPERIMENT (q7-board-name): what the board holds about each topic, written into
  ...
  const addTopic = action<AddTopicEvent, AddTopicResult>((
    ...
    // EXPERIMENT (q7-board-name): the name is allocated before the topic is
    // created, so the create can store it in the topic.
    const { name, member: piece } = createNamed(names, (allocated) => {
      // EXPERIMENT (q7-board-name): the entry document, minted by the create so the
      ...
      return Topic({
        ...
        mentionable,
        // EXPERIMENT (q7-board-name): this topic's own entry, and no table.
        ownEntry: entry,
