...
// packages/patterns/topics/topic.tsx with the edits
...
export interface TopicInput {
  ...
  /** EXPERIMENT (q7-board-name): the name the board allocated for this topic, stored
   * here by the create that filed it. */
  shortName?: string;

  /** EXPERIMENT (q7-board-name): this topic's own entry, the reference the board
...
/** EXPERIMENT (q7-board-name): what a topic shows of a topic that mentions it. */
export interface TopicBacklinkCopy {
  title: string;
  shortName: string;
}

/** EXPERIMENT (q7-board-name): what the board holds about one topic. */
export interface OwnEntry {
  /** The board's name for the topic this entry is about. */
  name: string;

  /** The topics that mention it, as copies. */
 ...
 */
/** EXPERIMENT (q7-board-name): the inbound references this topic's entry carries. */
const entryBacklinks = lift((
  { entry }: { entry: OwnEntry | undefined },
): TopicBacklinkCopy[] => (entry?.mentionedBy ?? []) as TopicBacklinkCopy[]);
...
export default pattern<TopicInput, TopicOutput>(
  (
    ...
    // EXPERIMENT (q7-board-name): the name is this topic's own stored input.
