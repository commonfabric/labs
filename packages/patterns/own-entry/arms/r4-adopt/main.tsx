// EXPERIMENT (own-entry, arm `r4-adopt`): a copy of
...
/** EXPERIMENT (r4-adopt): where the board keeps a topic's entry document,
...
export interface TopicsInput {
  /** EXPERIMENT (r4-adopt): one slot per topic the board has filed. */
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (r4-adopt): one slot per topic, each holding that topic's own
   * entry document. */
  ...
  /** EXPERIMENT (r4-adopt): how many references each per-topic fill wrote.
...
/** EXPERIMENT (r4-adopt): what the pivot reads of one topic — what it points
...
/** EXPERIMENT (r4-adopt): one row of the pivot, addressed by the topic it is
...
/** EXPERIMENT (r4-adopt): the board's mention pivot, named. One row per topic,
...
/** EXPERIMENT (r4-adopt): the pivot indexed by the name each row carries — a
...
/** EXPERIMENT (r4-adopt): ONE topic's entry, written from that topic's own row.
...
export default pattern<TopicsInput, TopicsOutput>((
  ...
  // EXPERIMENT (r4-adopt): the named pivot, indexed by name, and one entry per
