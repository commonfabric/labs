// EXPERIMENT (own-entry, arm `q4-handed`): a copy of
...
/** EXPERIMENT (q4-handed): where the board keeps a topic's entry document
...
export interface TopicsInput {
  /** EXPERIMENT (q4-handed): one slot per topic the board has filed. */
...
export interface TopicsOutput {
  ...
  /** EXPERIMENT (q4-handed): one slot per topic, each holding that topic's own
   * entry document. */
  entrySlots: EntrySlot[] | Default<[]>;

  /** EXPERIMENT (q4-handed): how many entries the last fill wrote. Demanding it
...
/** EXPERIMENT (q4-handed): what the board holds about each topic, written
...
export default pattern<TopicsInput, TopicsOutput>((
  ...
  // EXPERIMENT (q4-handed): what the board holds about each topic, written into
  ...
  const addTopic = action<AddTopicEvent, AddTopicResult>((
    ...
    // EXPERIMENT (q4-handed): the name is allocated before the topic is
    // created, so the create can store it in the topic.
    const { name, member: piece } = createNamed(names, (allocated) => {
      // EXPERIMENT (q4-handed): the entry document, minted by the create so the
      ...
      return Topic({
        ...
        mentionable,
        // EXPERIMENT (q4-handed): this topic's own entry, and no table.
        ownEntry: entry,
      });
    });
