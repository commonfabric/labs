...
export interface TopicInput {
  ...
  /** EXPERIMENT (q4-handed): the name the board allocated for this topic, stored
   * here by the create that filed it. */
  shortName?: string;

  /** EXPERIMENT (q4-handed): this topic's own entry, the reference the board
   * handed over when it filed the topic. */
  ownEntry?: ReadonlyCell<OwnEntry | undefined>;
}

/** EXPERIMENT (q4-handed): what the board holds about one topic. */
export interface OwnEntry {
  /** The board's name for the topic this entry is about. */
  name: string;

  /** The topics that mention it, as references. */
  mentionedBy: unknown[];
}
 ...
 */
/** EXPERIMENT (q4-handed): the inbound references this topic's entry carries. */
const entryBacklinks = lift((
  { entry }: { entry: OwnEntry | undefined },
): TopicSummary[] => (entry?.mentionedBy ?? []) as TopicSummary[]);
...
export default pattern<TopicInput, TopicOutput>(
  (
    ...
    // EXPERIMENT (q4-handed): the name is this topic's own stored input.
