...
describe("serving-memory-server", () => {
  describe("startServingMemoryServer()", () => {
    it("commits the consequence of an ON client's event, which a second client reads from the store", async () => {
      // Under ON the firing client commits only the event, and may show its
      // consequence speculatively. The second client has no overlay, so the
      // count it reads is what the serving loop committed.
