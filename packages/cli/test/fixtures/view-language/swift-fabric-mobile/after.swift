...
class LoomLocationPlugin: Plugin {
    ...
    @objc public func setConfig(_ invoke: Invoke) throws {
        let args = try invoke.parseArgs(SetConfigArgs.self)
        let cfg = BeaconConfig.shared
        cfg.ingestUrl = args.ingestUrl
        cfg.token = args.token
        if let id = args.installId, !id.isEmpty { cfg.installId = id }
        if let dev = args.devAllowLocalhost { cfg.devAllowLocalhost = dev }
        invoke.resolve(["ok": true, "configured": cfg.isConfigured])
    }
}
