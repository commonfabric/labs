...
describe("captured-cell-value-schema", () => {
  ...
  describe("a generic type", () => {
    ...
    it("emits the payload of a nongeneric alias whose payload substitution does not reach", async () => {
      // `Contact` names no parameter, and the lowering's expansion reaches
      // `Secret`'s `T` through an indexed access it does not substitute. The
      // type `Contact` instantiates holds the payload with `T` in.
