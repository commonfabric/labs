...
describe("the repository's gate suites", () => {
  ...
  it("gives the replay's own record to the suite rather than a unit", () => {
    // It measures the whole invocation, so adding its time to the time of
    // the fixtures inside it would count that work twice.
    expect(
  ...
  it("replays the fixtures a lane chose, and no others", async () => {
    // Each fixture restores its own store and compares against its own
    // manifest, so `--only` restricts the run to what a lane asked for.
    const vintage = byId("pattern-vintage");
  ...
  it("passes no filter where a lane asked for every fixture", async () => {
    // The whole-tree questions this gate also answers — whether every
    // required pattern is covered, and whether the exemption registry
    // still forgives anything — are only asked of an unfiltered run.
    const vintage = byId("pattern-vintage");
