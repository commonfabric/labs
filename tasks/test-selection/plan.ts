...
export interface PlanInput {
  ...
  /**
   * The units whose runner runs every identity in them whatever it is
   * asked, as `<suite>\t<unit>`. The topology names them, and
   * `wholeUnits` in `tasks/test-topology.ts` reads them off it.
   */
...
/**
 * What must run, keyed as the packer places it. A test that must run
 * makes its whole unit run; where two tests of one unit must run for
 * different reasons, the unit is placed under the first, and each test
 * is reported under its own when the plan is written.
 */
...
/** The corpus with each whole unit as one choice, and how to undo that. */
interface Folding {
...
/**
 * The corpus with every unit its runner runs entire made one entry.
 *
 * Such a unit offers one choice rather than many: a lane picking any test
 * in it runs all of them. Placed a test at a time, it would be charged a
 * fraction of what taking it spends, and several lanes picking different
 * tests in it would each run the lot. So the packer places the unit, and
 * what it placed is read back as the tests inside when the plan is
 * written, which keeps the fold from ever reaching a reader: the manifest
 * and every record name the tests, and so does the plan.
 *
 * The entry costs what its tests cost together, since that is what
 * running it spends. It carries the catches of all of them, because the
 * unit catches whenever any test in it does; two tests catching one
 * commit count twice, which errs toward running the unit, the direction
 * everything else here errs. Sources and churn take the largest, since
 * an entry carries how many distinct sources saw a catch rather than
 * which. The flake rate and the repeat count take the largest, because
 * one invocation disagreeing with itself is the unit disagreeing. Its
 * score is worked out from those inputs as of the day the manifest was
 * written, which is the day every other score in it is as of.
 *
 * A unit holding one identity is that identity, and is left alone. A row
 * the manifest carries twice is one identity, as it is everywhere else
 * here, so it is counted once.
 */
function foldWholeUnits(
  ...
  for (const group of grouped.values()) {
    ...
    // Named for the suite and the unit, which no record is: a record is
    // named for a test. The name lives only inside this function's
    // caller, and it has to be unique there, since a second entry under
    // it would take this one's place and its tests would run nowhere.
    ...
    if (keys.has(key)) {
      throw new Error(
        `the choice folding ${first.suite} ${first.unit} is named ${key}, ` +
          `and something else in the corpus already is, so one of the two ` +
          `would run nowhere`,
      );
    }
    keys.add(key);
...
export function plan(input: PlanInput): Plan {
  ...
  /** The key the packer knows an identity by. */
  ...
  // A test held back holds its whole unit back with it, since running the
  // unit is what runs that test.
  ...
  for (const entry of manifest.entries) {
    ...
    // A whole unit is as many identities as it holds, which is what a
    // reader of the count is asking about.
    ...
    // A whole unit is reported as the tests inside it, each at what the
    // unit costs, because the unit is what nothing can hold.
  ...
  return {
    lanes: lanes.map((lane) => ({
      ...
      selections: lane.selections.flatMap((selection) => {
        ...
        // A test the change made mandatory keeps the reason it was made
        // so; the rest ran for the unit's. A full run requires every test
        // for the one reason, whatever the caller also passed.
