...
| Clause      | CFC section                   | Relation   | Notes                                                                                                                                                                                                                                 |
| ----------- | ----------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AH-CFC-18` | §18.2.4.2, §18.2.4.3, §8.13   | `faithful` | §18.2.4.2: handle substitution "is itself an information flow and MUST be enforced as a read". §18.2.4.3: substitution "MUST respect ceilings". The converse — that minting or returning a handle is not a release — is §18.2.4.2's transfer rule: "a caller may pass the token as an opaque capability, but doing so does not reveal the payload." The host-side fit uses the runner's complete sink decision while remaining non-consuming, preserving §8.13's atomic treatment of single-use release. |
