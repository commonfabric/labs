...
- **The wait's own report** (`describeConditionWaitFailure`, in
  ...
  response status, the view and identity `globalThis.app` holds, the
  messages the runtime worker has logged at `warn` or `error` with their
  counts, and the browser console tail. The worker's own console does not
  reach the page, so those counts are the report's view of the worker. Every `waitForCondition` renders this, in every suite;
