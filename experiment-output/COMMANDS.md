...
The questions are
`/Users/mike/projects/labs-throwaway/collection-naming-2026-09-17/stages/own-entry/questions-round2.md`.
Round 2 adds seven arms, all generated by `make-arms.ts` in the same way:
`r1-pruned` (universe bounded to 50), `r1-lazy` (universe read by the
editor's open verb rather than at startup), `r2-per-entry` (entries filled one
per topic), `r3-one-entry` (everything the topic reads of its board in one
entry), `r4-adopt` (the create hands nothing; the entry is handed afterwards),
`r5-best` (per-topic fill plus a bounded universe as its own input), and
`r6-table-copies` (no handed reference: one table of values, read whole).

## The round-2 ladder (`experiment-output/round2/`)

```
for n in 4 40 200; do
  for a in r1-pruned r1-lazy r2-per-entry r3-one-entry r5-best \
           r6-table-copies; do
    CF_MEMORY_FRAME_LOG=$PWD/experiment-output/round2/live-$a-$n.jsonl \
      deno run -A packages/patterns/own-entry/measure-start.ts \
        --arm=$a --n=$n --d=2 --defer-demand=true \
        --out=experiment-output/round2 \
        >/dev/null 2>experiment-output/round2/$a-N$n.stderr.txt
  done
done
```

(N=4 was run without `--defer-demand`; the N=40 and N=200 runs use it. Each
run's record carries its own arguments.)

## N = 1000 (`experiment-output/round2-1000/`)

`--synthesize=true` writes every topic past the measured few straight to
storage — one document, one namespace entry and one entry slot each — instead
of filing them through `addTopic`, which is what makes N=1000 affordable.

```
for a in q6-copies r1-pruned r1-lazy r3-one-entry r5-best r6-table-copies; do
  CF_MEMORY_FRAME_LOG=$PWD/experiment-output/round2-1000/live-$a.jsonl \
    deno run -A packages/patterns/own-entry/measure-start.ts \
      --arm=$a --n=1000 --d=2 --synthesize=true --defer-demand=true \
      --out=experiment-output/round2-1000 \
      >/dev/null 2>experiment-output/round2-1000/$a-N1000.stderr.txt
done
```

**What synthesis is faithful for, and what it is not.**
`experiment-output/synth-check/` and `experiment-output/synth-control/` are the
same arm at N=20 built both ways:

```
deno run -A packages/patterns/own-entry/measure-start.ts --arm=r5-best \
  --n=20 --d=2 --synthesize=true --defer-demand=true \
  --out=experiment-output/synth-check
deno run -A packages/patterns/own-entry/measure-start.ts --arm=r5-best \
  --n=20 --d=2 --defer-demand=true --out=experiment-output/synth-control
```

The start delivers 176 documents either way, 152,686 against 152,690 bytes,
with the same attribution and the same checks — so a startup figure taken on a
synthesized board stands. A BOARD-SIDE figure does not always: a synthesized
entry already holds the value a whole-table refill would write, so the refill
writes nothing, and `q6-copies` and `r1-pruned` report 6 operations for a
mention change at N=1000 against 601 at N=200. The N=200 figures are the ones
the report uses for board cost.

## Handing an entry to a topic filed before the design

```
for route in verb operator; do
  CF_MEMORY_FRAME_LOG=$PWD/experiment-output/adopt/live-$route.jsonl \
    deno run -A packages/patterns/own-entry/measure-start.ts \
      --arm=r4-adopt --n=40 --d=2 --adopt=$route --defer-demand=true \
      --out=experiment-output/adopt \
      >/dev/null 2>experiment-output/adopt/$route-N40.stderr.txt
done
```

Each run's record was copied to `r4-adopt-<route>-N40.result.json` so both
survive; `experiment-output/summary-adopt.txt` is the table.
`experiment-output/adopt-control/` is the same arm with no adoption at all,
which settles and shows the topics holding no entry (`referencedBy: 0`).

## The broken runs of round 2

**The board verb that mints every missing entry does not settle.** The first
version of the adoption measurement called `r4-adopt`'s own `backfillEntries`
verb, which mints an entry and pushes a slot for every named topic in one
transaction. Both routes then hung in a non-settling scheduler loop naming 38
`fillOneEntry` actions ("Reactive graph did not settle within a scheduler
pass; retrying with backoff") and were killed:

```
deno run -A packages/patterns/own-entry/measure-start.ts --arm=r4-adopt \
  --n=40 --d=2 --adopt=verb --defer-demand=true --out=experiment-output/adopt
```

`experiment-output/adopt/verb-N40.stderr.txt` holds that warning and the abort
that followed; the control run in `adopt-control/` shows the same arm settling
when nothing calls the verb, so the loop is the verb's and not the arm's. The
measurement was then taken with the rig minting one entry in one transaction
instead — the per-topic figures the report quotes are unaffected, and the bulk
minting step is what remains unmeasured.

## The tables

```
deno run -A packages/patterns/own-entry/summarize.ts experiment-output/round2 \
  > experiment-output/summary-round2.txt
deno run -A packages/patterns/own-entry/summarize.ts \
  experiment-output/round2-1000 > experiment-output/summary-round2-1000.txt
deno run -A packages/patterns/own-entry/summarize.ts experiment-output/adopt \
  > experiment-output/summary-adopt.txt
deno run -A packages/patterns/own-entry/summarize.ts \
  experiment-output/synth-check experiment-output/synth-control \
  > experiment-output/summary-synthesis-check.txt
deno run -A packages/patterns/own-entry/show-frames.ts \
  experiment-output/round2/r3-one-entry-N200.start.jsonl --selectors=2 \
  > experiment-output/frames-r3-one-entry-N200-start.txt
```
